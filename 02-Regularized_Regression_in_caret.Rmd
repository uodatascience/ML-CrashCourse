...
```{r tidy_text_descriptions, cache = TRUE}
...
```{r get_description_sentiments, cache = TRUE}
...
```{r avg_fit_across_training_ridge, cache = TRUE}
...
Okay, so an $\bar{R^2}$ of `r round(avg_r2_ridge, 2)`, meaning we are explaining `r paste0(round(avg_r2_ridge*100, 2), "%")` of the variance in wine ratings with sentiment and price (and the interaction).

2) Harder test: how well does it do with the holdout sample?

```{r fit_predicting_holdout_ridge, cache = TRUE}
...
fitstat_ridge
```
Okay, so an $R^2$ of `r round(fitstat_ridge[["Rsquared"]], 2)`, meaning we are explaining `r paste0(round(fitstat_ridge[["Rsquared"]]*100, 2), "%")` of the variance in wine ratings with sentiment and price (and the interaction).
...
```{r avg_fit_across_training_lasso, cache = TRUE}
...
avg_RMSE_lasso
```
Okay, so an $\bar{R^2}$ of `r round(avg_r2_lasso, 2)`, meaning we are explaining `r paste0(round(avg_r2_lasso*100, 2), "%")` of the variance in wine ratings with sentiment and price (and the interaction).

Now for the harder test.

Harder test: how does it do with the holdout sample?

```{r fit_holdout_lasso, cache = TRUE}
...
Okay, so an $R^2$ of `r round(fitstat_lasso[["Rsquared"]], 2)`, meaning we are explaining `r paste0(round(fitstat_lasso[["Rsquared"]]*100, 2), "%")` of the variance in wine ratings with sentiment and price (and the interaction).
...
```{r avg_fit_across_trainin_enet, cache = TRUE}
...
avg_RMSE_enet
```
Okay, so an $\bar{R^2}$ of `r round(avg_r2_enet, 2)`, meaning we are explaining `r paste0(round(avg_r2_lasso*100, 2), "%")` of the variance (on average) in wine ratings with sentiment and price (and the interaction).


Harder test: how does it do with the holdout sample?

```{r fit_holdout_enet, cache = TRUE}
...
Okay, so an $R^2$ of `r round(fitstat_enet[["Rsquared"]], 2)`, meaning we are explaining `r paste0(round(fitstat_enet[["Rsquared"]]*100, 2), "%")` of the variance in wine ratings with sentiment and price (and the interaction).
